...
const assertSameAsMergeRight = (left, right) => {
  assert.deepEqual(
    objects.mergeRight(R.F, left, right),
...
describe('Deep objects:', () => {
  describe('Deep values:', () => {
    ...
    it('Value objects', () => {
      ...
      assert.deepEqual(objects.values(isValue, { a: { a: { a: 1 } } }), [
        { a: 1 }
      ]);
      assert.deepEqual(objects.values(isValue, { a: { a: { a: { a: 1 } } } }), [
        { a: 1 }
      ]);
    });
  });
  ...
  describe('Deep map:', () => {
    ...
    it('Nested objects', () => {
      assert.deepEqual(
        objects.map(R.F, a => a + 1, { a: { b: 1 } }),
        { a: { b: 2 } }
      );
      assert.deepEqual(
        objects.map(R.F, a => a + 1, { a: { b: { c: 1 } } }),
        { a: { b: { c: 2 } } }
      );
    });
  });
  ...
  describe('Deep mergeRight:', () => {
    ...
    it('Nested objects', () => {
      assert.deepEqual(
        objects.mergeRight(R.F, { a: { b: 1 } }, { a: { b: 2 } }),
        { a: { b: 2 } }
      );
    });
    ...
    it('Arrays', () => {
      ...
      assert.deepEqual(objects.mergeRight(R.F, { a: [1, 2] }, { a: [2] }), {
        a: [2, 2]
      });
      assert.deepEqual(objects.mergeRight(R.F, { a: [2, 2] }, { a: [1] }), {
        a: [1, 2]
      });
    });
  ...
  describe('TreeFlat:', () => {
    ...
    it('Nested objects', () => {
      ...
      assert.deepEqual(objects.treeFlat('_', R.F, { a: { b: { c: 1 } } }), {
        a_b_c: 1
      });

      assert.deepEqual(
        objects.treeFlat('_', R.F, { a: { c: 1 }, b: { c: 1 } }),
        { a_c: 1, b_c: 1 }
      );
    });
    ...
    it('Nested objects with arrays', () => {
      ...
      assert.deepEqual(objects.treeFlat('_', R.F, { a: { b: [1] } }), {
        a_b_0: 1
      });
    });
  });
