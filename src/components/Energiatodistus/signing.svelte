<script>
  ...
  Future.fork(
    response => { mPollux = R.assoc('failure', Maybe.Some(response), mPollux);  },
    response => { mPollux = R.assoc('version', Maybe.Some(response), mPollux); },
  ...
  const setStateStatus = (status) => setState(status,
  ...
  setStateStatus(Objects.requireNotNil(
    initialStatus[energiatodistus['tila-id']],
    'Energiatodistus ' + energiatodistus.id +
    ' invalid tila: ' + ET.tilaKey(energiatodistus['tila-id'])))();
  ...
  const statusText = state => {
    const languageAdjectiveName =
      $_('energiatodistus.signing.language-adjective.' + state.language)
    const languageAdjectiveGenetiveName =
      $_('energiatodistus.signing.language-genitive.' + state.language)
  ...
  const signPdf = language => R.compose(
    R.chain(etApi.signPdf(fetch, energiatodistus.versio, energiatodistus.id, language)),
  ...
  const signAllPdf = Kielisyys.bilingual(energiatodistus) ?
    ...
    (Kielisyys.onlySv(energiatodistus) ? signPdf('sv') : signPdf('fi'));
  ...
  const abort = _ => {
    cancel();
    Future.fork(
      _ => { error = Maybe.Some($_('energiatodistus.signing.error.abort-failed')) },
  ...
  <div class="content">
    ...
    <div class="buttons">
      ...
      <div class="mr-10 mt-5">
        <Button text={$_('energiatodistus.signing.button.start')} on:click={sign}/>
      </div>
      {/if}
      {#if !R.includes(currentState.status, closeTasks)}
      <div class="mt-5">
        <Button text={$_('energiatodistus.signing.button.abort')}
                style={'secondary'} on:click={abort}/>
      </div>
      {/if}
      {#if R.includes(currentState.status, closeTasks)}
      <div class="mt-5">
        <Button text={$_('energiatodistus.signing.button.close')}
                style={'secondary'} on:click={reload}/>
      </div>
      {/if}
    </div>
  </div>
