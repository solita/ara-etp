<script>
  ...
  const addApi = {
    save: virhetype =>
      api.add(serialize(R.dissoc('id', virhetype)), api.reload),
    cancel: _ => {
  ...
  <tbody class="etp-table--tbody">
    {#each Maybe.toArray(newVirhetype) as virhetype}
      <tr class="etp-table--tr">
        <td colspan="4">
          <TypeForm
            virhetype={R.assoc('id', Maybe.None(), toForm(virhetype))}
            bind:dirty
            api={addApi} />
        </td>
      </tr>
    ...
    {#each virhetypes as virhetype (virhetype.id)}
      {#if !Maybe.exists(R.equals(virhetype.id), virheInEditMode)}
        <tr
          class="etp-table--tr"
          class:etp-table--tr__link={!dirty}
      ...
      {:else}
        <tr class="etp-table--tr">
          <td colspan="4">
            <TypeForm virhetype={toForm(virhetype)} bind:dirty api={editApi} />
          </td>
        </tr>
      {/if}
    {/each}
