...
(t/deftest validation-laatimisvaihe-2018-rejects-id-3
  ...
  ;; then an error is thrown indicating invalid laatimisvaihe
  (let [{:keys [laatijat]} (test-data-set)
        laatija-id (-> laatijat keys sort first)
        energiatodistus (-> (energiatodistus-test-data/generate-add 2018 false)
                            (assoc-in [:perustiedot :laatimisvaihe] 3))]
    (t/is (= :invalid-laatimisvaihe
...
(t/deftest validation-laatimisvaihe-2018-rejects-id-4
  ;; given a valid 2018 energiatodistus with laatimisvaihe set to 4,
  ;; when attempting to save it,
  ;; then an error is thrown indicating invalid laatimisvaihe
  (let [{:keys [laatijat]} (test-data-set)
        laatija-id (-> laatijat keys sort first)
...
(t/deftest validation-laatimisvaihe-2026-accepts-id-3
  ;; given a valid 2026 energiatodistus with laatimisvaihe set to 3,
  ;; when saving it,
  ;; then no exception is thrown and the energiatodistus is created
  (let [{:keys [laatijat]} (test-data-set)
        laatija-id (-> laatijat keys sort first)
        energiatodistus (-> (energiatodistus-test-data/generate-add 2026 false)
                            (assoc-in [:perustiedot :laatimisvaihe] 3))]
...
(t/deftest validation-laatimisvaihe-2026-accepts-id-4
  ;; given a valid 2026 energiatodistus with laatimisvaihe set to 4,
  ;; when saving it,
  ;; then no exception is thrown and the energiatodistus is created
  (let [{:keys [laatijat]} (test-data-set)
        laatija-id (-> laatijat keys sort first)
...
;; --- Regression: existing laatimisvaihe IDs still accepted ---

(t/deftest validation-laatimisvaihe-2018-accepts-original-ids
  ;; given valid 2018 energiatodistukset with laatimisvaihe 0, 1, and 2,
  ;; when saving each one,
  ;; then all are accepted without errors
  (let [{:keys [laatijat]} (test-data-set)
        laatija-id (-> laatijat keys sort first)]
    (doseq [vaihe-id [0 1 2]]
      (let [energiatodistus (-> (energiatodistus-test-data/generate-add 2018 false)
                                (assoc-in [:perustiedot :laatimisvaihe] vaihe-id))]
        (t/is (number? (:id (service/add-energiatodistus!
                              (ts/db-user laatija-id)
                              (test-whoami/laatija laatija-id)
                              2018
...
(t/deftest validation-laatimisvaihe-2026-accepts-original-ids
  ;; given valid 2026 energiatodistukset with laatimisvaihe 0, 1, and 2,
  ;; when saving each one,
  ;; then all are accepted without errors
  (let [{:keys [laatijat]} (test-data-set)
        laatija-id (-> laatijat keys sort first)]
    (doseq [vaihe-id [0 1 2]]
      (let [energiatodistus (-> (energiatodistus-test-data/generate-add 2026 false)
                                (assoc-in [:perustiedot :laatimisvaihe] vaihe-id))]
        (t/is (number? (:id (service/add-energiatodistus!
                              (ts/db-user laatija-id)
                              (test-whoami/laatija laatija-id)
