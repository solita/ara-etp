...
(t/deftest ^{:broken-on-windows-test "Couldn't delete .. signable.pdf"} test-palveluvayla-api
  (let [; Add laatija
        ...
        todistus-2026-fi (-> (test-data.energiatodistus/generate-add 2026 true) (assoc-in [:perustiedot :rakennustunnus] rakennustunnus-1) (assoc-in [:perustiedot :kieli] 0))
        ...
        todistus-2026-sv (-> (test-data.energiatodistus/generate-add 2026 true) (assoc-in [:perustiedot :rakennustunnus] rakennustunnus-1) (assoc-in [:perustiedot :kieli] 1))
        ...
        todistus-2026-multilingual (-> (test-data.energiatodistus/generate-add 2026 true) (assoc-in [:perustiedot :rakennustunnus] rakennustunnus-1) (assoc-in [:perustiedot :kieli] 2))
        ...
        todistus-2026-rakennustunnus-2 (-> (test-data.energiatodistus/generate-add 2026 true) (assoc-in [:perustiedot :rakennustunnus] rakennustunnus-2))

        ; Insert all energiatodistus
        [todistus-2013-fi-id todistus-2018-fi-id todistus-2026-fi-id todistus-2013-sv-id todistus-2018-sv-id todistus-2026-sv-id todistus-2013-multilingual-id todistus-2018-multilingual-id todistus-2026-multilingual-id todistus-2013-rakennustunnus-2-id todistus-2018-rakennustunnus-2-id todistus-2026-rakennustunnus-2-id old-todistus-2013-fi-id ] (test-data.energiatodistus/insert! [todistus-2013-fi todistus-2018-fi todistus-2026-fi todistus-2013-sv todistus-2018-sv todistus-2026-sv todistus-2013-multilingual todistus-2018-multilingual todistus-2026-multilingual todistus-2013-rakennustunnus-2 todistus-2018-rakennustunnus-2 todistus-2026-rakennustunnus-2 old-todistus-2013-fi] laatija-id)]

    ; Sign all energiatodistus
    ; LibreOffice is quite slow so do signing in parallel
    (doall (pmap #(test-data.energiatodistus/sign! % laatija-id false) [todistus-2013-fi-id todistus-2018-fi-id todistus-2013-sv-id todistus-2018-sv-id todistus-2013-multilingual-id todistus-2018-multilingual-id todistus-2013-rakennustunnus-2-id todistus-2018-rakennustunnus-2-id old-todistus-2013-fi-id old-todistus-2013-fi-id todistus-2026-fi-id todistus-2026-sv-id todistus-2026-multilingual-id todistus-2026-rakennustunnus-2-id]))
    ...
    (t/testing "Fetching energiatodistus by id returns it when fetching correct version"
      ...
      (t/testing "when fetching 2026"
        (let [response (ts/handler (-> (mock/request :get (str palveluvayla-basepath (format "/json/2026/%s" todistus-2026-fi-id)))))
              body (j/read-value (:body response) j/keyword-keys-object-mapper)]
          (t/is (= 200 (:status response)))
          (t/is (= todistus-2026-fi-id (:id body)))
          (schema-tools.coerce/coerce body schema.energiatodistus/Energiatodistus2026 schema-tools.coerce/json-coercion-matcher))))
    ...
    (t/testing "Fetching wrong version by id returns 404"
      ...
      (t/testing "for 2026"
        (let [response (ts/handler (-> (mock/request :get (str palveluvayla-basepath (format "/json/2026/%s" todistus-2018-fi-id)))))]
          (t/is (= 404 (:status response))))))
    ...
    (t/testing "Fetching pdf"
      ...
      (t/testing "for version 2018"
        ...
        (t/testing "for version 2026"
          (let [response (ts/handler (-> (mock/request :get (str palveluvayla-basepath (format "/pdf/%s" todistus-2026-fi-id)))))
                body (-> response :body io/input-stream)
                correct-pdf (file/find-file ts/*aws-s3-client* (format "energiatodistukset/energiatodistus-%s-%s" todistus-2026-fi-id "fi"))]
            (t/is (= 200 (:status response)))
            (t/is (IOUtils/contentEquals body correct-pdf)))))
      ...
      (t/testing "Accept-Language is honored"
        ...
        (t/testing "when asking for existing Finnish pdf"
          (let [response (ts/handler (-> (mock/request :get (str palveluvayla-basepath (format "/pdf/%s" todistus-2026-multilingual-id)))
                                         (mock/header "Accept-Language" "fi")))
                body (-> response :body io/input-stream)
                correct-pdf (file/find-file ts/*aws-s3-client* (format "energiatodistukset/energiatodistus-%s-%s" todistus-2026-multilingual-id "fi"))]
            (t/is (= 200 (:status response)))
            (t/is (IOUtils/contentEquals body correct-pdf))))
        ...
        (t/testing "when asking for existing Swedish pdf"
          (let [response (ts/handler (-> (mock/request :get (str palveluvayla-basepath (format "/pdf/%s" todistus-2026-multilingual-id)))
                                         (mock/header "Accept-Language" "sv")))
                body (-> response :body io/input-stream)
                correct-pdf (file/find-file ts/*aws-s3-client* (format "energiatodistukset/energiatodistus-%s-%s" todistus-2026-multilingual-id "sv"))]
            (t/is (= 200 (:status response)))
            (t/is (IOUtils/contentEquals body correct-pdf))))
    ...
    (t/testing "Searching for specific version by rakennustunnus returns only that version"
      ...
      (t/testing "when searching for 2026"
        (let [response (ts/handler (-> (mock/request :get (str palveluvayla-basepath (format "/json/2026?rakennustunnus=%s" rakennustunnus-1)))))
              body (j/read-value (:body response) j/keyword-keys-object-mapper)]
          (t/is (= 200 (:status response)))
          (t/is (= 3 (count body)))
          (t/is (= #{todistus-2026-fi-id
                     todistus-2026-sv-id
                     todistus-2026-multilingual-id}
                   (set (map :id body))))
          (schema-tools.coerce/coerce body [schema.energiatodistus/Energiatodistus2026] schema-tools.coerce/json-coercion-matcher))))))
