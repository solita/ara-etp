  ...
  {:korvattu-energiatodistus-id          nil
   ...
   {:lammitetty-nettoala 5000
    ...
    {:pohjoinen {:ala 0   :U 1   :g-ks 0.75}
     ...
     :valokupu  {:ala 0   :U 0.5 :g-ks 0.8}
     :katto     {:ala 1   :U 1   :g-ks 0.8}}
    :ilmanvaihto
...
(def energiatodistus-2026
  {:korvattu-energiatodistus-id          nil
   ...
   {:lammitetty-nettoala 3000
    ...
    :lkvn-kaytto {:ominaiskulutus 40 :lammitysenergian-nettotarve 40}
...
;; ---------------------------------------------------------------------------
;; Resource directory for the CSV files
;; ---------------------------------------------------------------------------

(def ^:private resource-dir "src/dev/clj/solita/etp/")
...
(defn create-aineistot-with-default-data!
  "Insert energiatodistukset via API, sign them via API, and regenerate aineistot.
   Returns the inserted ids.

   Notice that you need to set duration of a valid session to some really long time. Or alternatively
   you can just temporarily go and set the `solita.etp.security/session-fresh?` function to return true."
  []
